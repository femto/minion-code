...
class MinionCodeToolsGradio:
    ...
    def create_interface(self):
        ...
        with gr.Blocks(title="Minion Code Tools", theme=gr.themes.Soft()) as interface:
            ...
            gr.Markdown("A powerful collection of development tools for code analysis and manipulation.")
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("📁 File Operations"):
                    with gr.Row():
                        with gr.Column():
                            ...
                            read_file_path = gr.Textbox(label="File Path", placeholder="path/to/file.txt")
                            read_offset = gr.Number(label="Offset (optional)", value=None, precision=0)
                            read_limit = gr.Number(label="Limit (optional)", value=None, precision=0)
                            read_btn = gr.Button("Read File", variant="primary")
                            read_output = gr.Textbox(label="Output", lines=10, max_lines=20)
                        
                        with gr.Column():
                            gr.Markdown("### Write File")
                            write_file_path = gr.Textbox(label="File Path", placeholder="path/to/file.txt")
                            write_content = gr.Textbox(label="Content", lines=5, placeholder="File content...")
                ...
                with gr.TabItem("📂 Directory Operations"):
                    with gr.Row():
                        with gr.Column():
                            ...
                            ls_path = gr.Textbox(label="Directory Path", value=".", placeholder=".")
                            ls_recursive = gr.Checkbox(label="Recursive", value=False)
                            ls_btn = gr.Button("List Directory", variant="primary")
                            ls_output = gr.Textbox(label="Output", lines=15, max_lines=25)
                        
                        with gr.Column():
                            gr.Markdown("### Find Files (Glob)")
                            glob_pattern = gr.Textbox(label="Pattern", placeholder="*.py")
                            glob_path = gr.Textbox(label="Search Path", value=".", placeholder=".")
                            glob_btn = gr.Button("Find Files", variant="primary")
                            glob_output = gr.Textbox(label="Output", lines=15, max_lines=25)
                
                # Search Tab
                with gr.TabItem("🔍 Search"):
                    gr.Markdown("### Search Text in Files (Grep)")
                    with gr.Row():
                        grep_pattern = gr.Textbox(label="Search Pattern", placeholder="import")
                        grep_path = gr.Textbox(label="Search Path", value=".", placeholder=".")
                        grep_include = gr.Textbox(label="File Pattern (optional)", placeholder="*.py")
                    grep_btn = gr.Button("Search", variant="primary")
                    grep_output = gr.Textbox(label="Search Results", lines=15, max_lines=30)
                
                # Code Execution Tab
                with gr.TabItem("🐍 Python Execution"):
                    gr.Markdown("### Execute Python Code")
                    python_code = gr.Code(label="Python Code", language="python", lines=10)
                ...
                with gr.TabItem("💻 Bash Commands"):
                    ...
                    gr.Markdown("⚠️ **Warning**: Only safe commands are allowed. Dangerous commands are blocked.")
                    bash_command = gr.Textbox(label="Command", placeholder="echo 'Hello World'")
                    bash_timeout = gr.Number(label="Timeout (seconds)", value=30, precision=0)
                    bash_btn = gr.Button("Execute Command", variant="primary")
                    bash_output = gr.Textbox(label="Output", lines=10, max_lines=20)
            
            # Event handlers
            read_btn.click(
                fn=lambda path, offset, limit: self.process_tool_request('read', path, offset, limit),
                inputs=[read_file_path, read_offset, read_limit],
                outputs=read_output
            )
            
            write_btn.click(
                fn=lambda path, content: self.process_tool_request('write', path, content),
                inputs=[write_file_path, write_content],
                outputs=write_output
            )
            
            ls_btn.click(
                fn=lambda path, recursive: self.process_tool_request('ls', path, recursive),
                inputs=[ls_path, ls_recursive],
                outputs=ls_output
            )
            
            glob_btn.click(
                fn=lambda pattern, path: self.process_tool_request('glob', pattern, path),
                inputs=[glob_pattern, glob_path],
                outputs=glob_output
            )
            
            grep_btn.click(
                fn=lambda pattern, path, include: self.process_tool_request('grep', pattern, path, include if include else None),
            ...
            bash_btn.click(
                fn=lambda command, timeout: self.process_tool_request('bash', command, timeout),
