#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Text search tool."""

import re
from pathlib import Path
from typing import Any, List, Optional

from minion.tools import BaseTool

from ..utils.output_truncator import truncate_output
from ..utils.search_backend import DEFAULT_TOOL_RESULT_LIMIT
from ..utils.search_backend import build_rg_exclude_args
from ..utils.search_backend import collect_rg_lines
from ..utils.search_backend import find_rg
from ..utils.search_backend import run_rg
from ..utils.search_backend import should_skip_relative_path


class GrepTool(BaseTool):
    """Search for text patterns in files."""

    name = "grep"
    description = "Search for text patterns in files"
    readonly = True
    inputs = {
        "pattern": {
            "type": "string",
            "description": "Regular expression pattern to search for",
        },
        "path": {"type": "string", "description": "Search path (file or directory)"},
        "include": {
            "type": "string",
            "description": "File pattern to include (optional)",
            "nullable": True,
        },
        "output_mode": {
            "type": "string",
            "description": "Output mode: 'content' (show matching lines), 'files_with_matches' (show file paths), 'count' (show match counts)",
            "nullable": True,
        },
        "head_limit": {
            "type": "integer",
            "description": "Limit output to first N entries",
            "nullable": True,
        },
        "after_context": {
            "type": "integer",
            "description": "Number of lines to show after each match (-A)",
            "nullable": True,
        },
        "before_context": {
            "type": "integer",
            "description": "Number of lines to show before each match (-B)",
            "nullable": True,
        },
        "context": {
            "type": "integer",
            "description": "Number of lines to show before and after each match (-C)",
            "nullable": True,
        },
    }
    output_type = "string"

    def __init__(self, workdir: Optional[str] = None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.workdir = Path(workdir) if workdir else None

    def _resolve_path(self, path: str) -> Path:
        """Resolve path using workdir if path is relative."""
        candidate = Path(path)
        if candidate.is_absolute():
            return candidate
        if self.workdir:
            return self.workdir / candidate
        return candidate

    def forward(
        self,
        pattern: str,
        path: str = ".",
        include: Optional[str] = None,
        output_mode: Optional[str] = None,
        head_limit: Optional[int] = None,
        after_context: Optional[int] = None,
        before_context: Optional[int] = None,
        context: Optional[int] = None,
    ) -> str:
        """Search for text pattern."""
        try:
            if output_mode is None:
                output_mode = "content"
            if output_mode not in {"content", "files_with_matches", "count"}:
                return (
                    f"Error: Invalid output_mode '{output_mode}'. Must be 'content', "
                    "'files_with_matches', or 'count'"
                )

            if head_limit is None:
                head_limit = DEFAULT_TOOL_RESULT_LIMIT
            if head_limit < 1:
                return "Error: head_limit must be greater than zero"

            if context is not None:
                after_context = context
                before_context = context

            search_path = self._resolve_path(path)
            if not search_path.exists():
                return f"Error: Path does not exist - {path}"

            rg_path = find_rg()
            if rg_path:
                result = self._search_with_rg(
                    pattern,
                    search_path,
                    include=include,
                    output_mode=output_mode,
                    head_limit=head_limit,
                    before_context=before_context,
                    after_context=after_context,
                )
                return self.format_for_observation(result)

            matches = self._search_with_python(
                pattern,
                search_path,
                include=include,
                before_context=before_context,
                after_context=after_context,
            )
            if not matches:
                return f"No content found matching pattern '{pattern}'"
            if output_mode == "files_with_matches":
                return self.format_for_observation(
                    self._format_files_with_matches(matches, pattern, head_limit)
                )
            if output_mode == "count":
                return self.format_for_observation(
                    self._format_count(matches, pattern, head_limit)
                )
            return self.format_for_observation(
                self._format_content(
                    matches,
                    pattern,
                    head_limit,
                    before_context,
                    after_context,
                )
            )
        except Exception as exc:
            return f"Error during search: {exc}"

    def _search_with_rg(
        self,
        pattern: str,
        search_path: Path,
        *,
        include: Optional[str],
        output_mode: str,
        head_limit: int,
        before_context: Optional[int],
        after_context: Optional[int],
    ) -> str:
        args = ["--no-messages", "--smart-case"]
        args.extend(build_rg_exclude_args())
        if include:
            args.extend(["-g", include])

        if output_mode == "files_with_matches":
            args.append("--files-with-matches")
        elif output_mode == "count":
            args.append("-c")
        else:
            args.extend(["-n", "--no-heading", "--color", "never"])
            if before_context:
                args.extend(["-B", str(before_context)])
            if after_context:
                args.extend(["-A", str(after_context)])

        args.extend(["--regexp", pattern, str(search_path)])

        if output_mode == "count":
            completed = run_rg(args, self.workdir or search_path.parent)
            if completed.returncode == 1:
                return f"No content found matching pattern '{pattern}'"
            if completed.returncode not in (0, 1):
                raise RuntimeError(completed.stderr.strip() or "rg failed")
            lines = [line for line in completed.stdout.splitlines() if line.strip()]
            if not lines:
                return f"No content found matching pattern '{pattern}'"
            result = [f"Match counts for pattern '{pattern}':", ""]
            result.extend(lines[:head_limit])
            if len(lines) > head_limit:
                result.extend(
                    [
                        "",
                        f"(Output limited to {head_limit} files)",
                    ]
                )
            result.extend(["", f"Total {len(lines)} files with matches"])
            return "\n".join(result)

        lines, truncated = collect_rg_lines(
            args,
            self.workdir or search_path.parent,
            max_results=head_limit,
        )
        if not lines:
            return f"No content found matching pattern '{pattern}'"

        if output_mode == "files_with_matches":
            result = [f"Files matching pattern '{pattern}':", ""]
            result.extend(lines)
            if truncated:
                result.extend(["", f"(Output limited to {head_limit} files)"])
            result.extend(["", f"Total {len(lines)} files with matches"])
            return "\n".join(result)

        result = [f"Search results for pattern '{pattern}':", ""]
        result.extend(lines)
        if truncated:
            result.extend(["", f"(Output limited to {head_limit} lines)"])
        result.extend(["", f"Displayed {len(lines)} matching lines"])
        return "\n".join(result)

    def _search_with_python(
        self,
        pattern: str,
        search_path: Path,
        *,
        include: Optional[str],
        before_context: Optional[int],
        after_context: Optional[int],
    ) -> List[tuple]:
        matches = []
        if search_path.is_file():
            matches.extend(
                self._search_file(search_path, pattern, before_context, after_context)
            )
            return matches

        if include:
            candidate_paths = search_path.rglob(include)
        else:
            candidate_paths = search_path.rglob("*")

        for file_path in candidate_paths:
            if not file_path.is_file():
                continue
            try:
                relative_path = file_path.relative_to(search_path)
            except ValueError:
                relative_path = Path(file_path.name)
            if should_skip_relative_path(relative_path):
                continue
            if include is None and not self._is_text_file(file_path):
                continue
            matches.extend(
                self._search_file(file_path, pattern, before_context, after_context)
            )
        return matches

    def _format_content(
        self,
        matches: List[tuple],
        pattern: str,
        head_limit: Optional[int],
        before_context: Optional[int] = None,
        after_context: Optional[int] = None,
    ) -> str:
        result = f"Search results for pattern '{pattern}':\n\n"
        current_file = None
        count = 0
        has_context = before_context or after_context

        for match in matches:
            if head_limit and count >= head_limit:
                result += f"\n(Output limited to {head_limit} matches)"
                break

            if has_context and len(match) == 5:
                file_path, line_num, line_content, before_lines, after_lines = match
            else:
                file_path, line_num, line_content = match[:3]
                before_lines = []
                after_lines = []

            if file_path != current_file:
                if current_file is not None and has_context:
                    result += "--\n"
                result += f"File: {file_path}\n"
                current_file = file_path

            for ctx_line_num, ctx_line in before_lines:
                result += f"  {ctx_line_num}-  {ctx_line.rstrip()}\n"
            result += f"  {line_num}:  {line_content.rstrip()}\n"
            for ctx_line_num, ctx_line in after_lines:
                result += f"  {ctx_line_num}-  {ctx_line.rstrip()}\n"
            if has_context:
                result += "--\n"
            count += 1

        result += f"\nTotal {len(matches)} matches found"
        return result

    def _format_files_with_matches(
        self, matches: List[tuple], pattern: str, head_limit: Optional[int]
    ) -> str:
        unique_files = []
        seen = set()
        for file_path, _, _ in matches:
            if file_path in seen:
                continue
            seen.add(file_path)
            unique_files.append(file_path)
            if head_limit and len(unique_files) >= head_limit:
                break

        result = f"Files matching pattern '{pattern}':\n\n"
        for file_path in unique_files:
            result += f"{file_path}\n"
        if head_limit and len(seen) > head_limit:
            result += f"\n(Output limited to {head_limit} files)"
        result += f"\nTotal {len(seen)} files with matches"
        return result

    def _format_count(
        self, matches: List[tuple], pattern: str, head_limit: Optional[int]
    ) -> str:
        file_counts = {}
        for file_path, _, _ in matches:
            file_counts[file_path] = file_counts.get(file_path, 0) + 1

        result = f"Match counts for pattern '{pattern}':\n\n"
        count = 0
        for file_path, match_count in file_counts.items():
            if head_limit and count >= head_limit:
                result += f"\n(Output limited to {head_limit} files)"
                break
            result += f"{file_path}: {match_count} matches\n"
            count += 1

        result += (
            f"\nTotal {sum(file_counts.values())} matches in {len(file_counts)} files"
        )
        return result

    def _search_file(
        self,
        file_path: Path,
        pattern: str,
        before_context: Optional[int] = None,
        after_context: Optional[int] = None,
    ) -> List[tuple]:
        matches = []
        try:
            lines = file_path.read_text(encoding="utf-8", errors="ignore").splitlines(
                keepends=True
            )
            total_lines = len(lines)
            for line_num, line in enumerate(lines, 1):
                if not re.search(pattern, line, re.IGNORECASE):
                    continue
                if before_context or after_context:
                    before_lines = []
                    if before_context:
                        start = max(0, line_num - 1 - before_context)
                        for index in range(start, line_num - 1):
                            before_lines.append((index + 1, lines[index]))

                    after_lines = []
                    if after_context:
                        end = min(total_lines, line_num + after_context)
                        for index in range(line_num, end):
                            after_lines.append((index + 1, lines[index]))

                    matches.append(
                        (str(file_path), line_num, line, before_lines, after_lines)
                    )
                else:
                    matches.append((str(file_path), line_num, line))
        except Exception:
            pass
        return matches

    def _is_text_file(self, file_path: Path) -> bool:
        text_extensions = {
            ".txt",
            ".py",
            ".js",
            ".ts",
            ".tsx",
            ".jsx",
            ".html",
            ".css",
            ".json",
            ".xml",
            ".md",
            ".yml",
            ".yaml",
            ".ini",
            ".cfg",
            ".conf",
        }
        return file_path.suffix.lower() in text_extensions

    def format_for_observation(self, output: Any) -> str:
        """Format output with truncation safeguards."""
        if isinstance(output, str):
            return truncate_output(output, tool_name=self.name)
        return str(output)
