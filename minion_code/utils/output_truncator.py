...
class FileTooLargeError(Exception):
    """File too large, suggest using specialized tool"""
    def __init__(
        self,
        message: str,
        file_path: str,
...
def check_file_size_before_read(
    ...
    Raises:
        FileTooLargeError: When file exceeds size limit, includes suggested tool
    """
    path = Path(file_path)
    if not path.exists():
        return
    ...
    if file_size > max_size:
        ...
        suggested = tool_suggestions.get(suffix, 'paginated read (offset/limit params)')
...
def truncate_output(
    ...
    """
    Truncate built-in tool output (auto-applied).

    Args:
        output: Original output
        max_size: Maximum bytes, default 400KB
        tool_name: Tool name for generating specific hints

    Returns:
        Truncated output with hint if truncation occurred
    """
    output_bytes = output.encode('utf-8')
    if len(output_bytes) <= max_size:
        return output

    # Truncate to max_size bytes, ensure UTF-8 characters are not split
...
def check_mcp_output(
    ...
    Returns:
        Original output if within limit

    Raises:
        MCPContentTooLargeError: When output exceeds token limit
    """
    ...
    if estimated_tokens > max_tokens:
        raise MCPContentTooLargeError(
            f"MCP tool output too large (~{estimated_tokens} tokens > {max_tokens} limit)",
...
def _get_tool_hint(tool_name: str) -> str:
    ...
    hints = {
        ...
        'grep': "Hint: Use `head_limit` param or more specific search pattern",
        'glob': "Hint: Use more specific pattern to narrow matches",
        'ls': "Hint: Avoid recursive mode or specify more specific subdirectory",
        'file_read': "Hint: Use `offset` and `limit` params for paginated read",
        'python': "Hint: Control print output in your code",
    }
    return hints.get(tool_name, "Hint: Use more specific params to reduce output")
