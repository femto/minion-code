...
# Type definitions
McpStdioServerConfig = TypedDict('McpStdioServerConfig', {
...
McpSSEServerConfig = TypedDict('McpSSEServerConfig', {
    'type': Literal['sse'],
    'url': str
})
...
NotificationChannel = Literal['iterm2', 'terminal_bell', 'iterm2_with_bell', 'notifications_disabled']
ProviderType = Literal[
    'anthropic', 'openai', 'mistral', 'deepseek', 'kimi', 'qwen', 'glm',
    'minimax', 'baidu-qianfan', 'siliconflow', 'bigdream', 'opendev',
    'xai', 'groq', 'gemini', 'ollama', 'azure', 'custom', 'custom-openai'
]
...
GLOBAL_CONFIG_KEYS = [
    'auto_updater_status', 'theme', 'has_completed_onboarding',
    'last_onboarding_version', 'last_release_notes_seen', 'verbose',
    'custom_api_key_responses', 'primary_provider', 'preferred_notif_channel',
    'shift_enter_key_binding_installed', 'max_tokens'
]

PROJECT_CONFIG_KEYS = [
    'dont_crawl_directory', 'enable_architect_tool',
    'has_trust_dialog_accepted', 'has_completed_project_onboarding'
]
...
class ConfigManager:
    ...
    def _is_test_env(self) -> bool:
        ...
        return os.getenv('PYTHON_ENV') == 'test' or os.getenv('PYTEST_CURRENT_TEST') is not None
    ...
    def _save_config(self, file_path: Path, config: Any, default_config: Any) -> None:
        ...
        for key, value in config_dict.items():
            if key in default_dict:
                if json.dumps(value, sort_keys=True) != json.dumps(default_dict[key], sort_keys=True):
    ...
    def _load_config(self, file_path: Path, default_config: Any, throw_on_invalid: bool = False) -> Any:
        ...
        try:
            ...
            if parsed_config is None:
                if throw_on_invalid:
                    raise ConfigParseError(f"Invalid JSON in {file_path}", str(file_path), default_config)
            ...
            logger.warning(f"Error loading config from {file_path}: {e}, using defaults")
            return deepcopy(default_config)
    ...
    def get_custom_api_key_status(self, truncated_api_key: str) -> Literal['approved', 'rejected', 'new']:
    ...
    def _migrate_model_profiles_remove_id(self, config: GlobalConfig) -> GlobalConfig:
        ...
        if config.model_pointers:
            pointers_dict = asdict(config.model_pointers) if hasattr(config.model_pointers, '__dataclass_fields__') else config.model_pointers
        ...
        if hasattr(config, 'default_model_id'):
            default_model_name = id_to_model_name.get(getattr(config, 'default_model_id'), getattr(config, 'default_model_id'))
    ...
    def validate_and_repair_gpt5_profile(self, profile: ModelProfile) -> ModelProfile:
        ...
        if is_gpt5:
            ...
            if not profile.reasoning_effort or profile.reasoning_effort not in valid_reasoning_efforts:
                repaired_profile.reasoning_effort = 'medium'
                was_repaired = True
                logger.info(f"🔧 GPT-5 Config: Set reasoning effort to 'medium' for {profile.model_name}")
            
            # Context length validation
            if profile.context_length < 128000:
                repaired_profile.context_length = 128000
                was_repaired = True
                logger.info(f"🔧 GPT-5 Config: Updated context length to 128k for {profile.model_name}")
            
            # Output tokens validation
            if profile.max_tokens < 4000:
                repaired_profile.max_tokens = 8192
                was_repaired = True
                logger.info(f"🔧 GPT-5 Config: Updated max tokens to 8192 for {profile.model_name}")
            
            # Base URL validation
            if 'gpt-5' in profile.model_name and not profile.base_url:
                repaired_profile.base_url = 'https://api.openai.com/v1'
                was_repaired = True
                logger.info(f"🔧 GPT-5 Config: Set default base URL for {profile.model_name}")
        
        # Update validation metadata
        repaired_profile.validation_status = 'auto_repaired' if was_repaired else 'valid'
        repaired_profile.last_validation = now
        
        if was_repaired:
            logger.info(f"✅ GPT-5 Config: Auto-repaired configuration for {profile.model_name}")
